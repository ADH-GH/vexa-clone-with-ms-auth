...
export async function joinMicrosoftTeams(
  ...
  if (botConfig.authenticated) {
    // Signed-in mode: the persistent context carries automation@'s Entra session, so
    // Teams renders the pre-join with our real identity and NO name field. A signed-in
    // org member is admitted by policy — no anonymous guest, no lobby gate. Mirrors the
    // authenticated branch in googlemeet/join.ts.
    log("Step 4: Authenticated mode — skipping display-name input (signed-in identity).");
  } else {
